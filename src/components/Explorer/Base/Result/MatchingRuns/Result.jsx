...
const MatchingRunsResult = ({
    ...
    const [
        searchLevelValueCorrected,
        setSearchLevelValueCorrected,
    ] = React.useState(searchLevelValue)
    const LinkButtons = context.result.LinkButtons

    React.useEffect(() => {
        if (!searchLevelValue) return
        console.log(
            `Loading search result page for ${searchLevel}=${searchLevelValue}.`
        )
